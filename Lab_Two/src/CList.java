...
//        Test the sort() method
    ...
    *   Input:      N/A
    *   Output:     N/A
    *   Purpose:    Cut out all elements between the first element and the last
    *               occurrence of the first element.
    */
    public void cut() {
//        Since the first item is also the last item (so far) instantiate as such
        node lastOccurrence = first;
//        Go through all nodes and if the node item is the same as the head's item, mark the
//        last occurrence as that node
        for (node i = first; i != null; i = i.next) {
            if (i.item.equals(lastOccurrence.item))
                lastOccurrence = i;
        }
//        If there are no duplicate items, return, since nothing can be cut out
        if (lastOccurrence.equals(first))
            return;
//        Otherwise go through the CList (until the last occurrence), popping each node
        for (node i = first; i != lastOccurrence; i = i.next) {
            pop();
        }
    }
